...
public class RefreshListView extends ListView implements AbsListView.OnScrollListener{
     ...
     */
    public void stopLoadMore(int hasData) {
        if (mPullLoading) {
            mPullLoading = false;
            mFooterView.setState(RefreshListViewFooter.STATE_NORMAL);
        }
        if (hasData == 1) {//还有数据  显示“加载更多”
            mFooterView.setTextForTips("查看更多");
        } else if (hasData == 0) {//没有数据  显示“已加载全部数据”
            mFooterView.setTextForTips("已加载全部");
        } else if (hasData == -1) {
            mFooterView.setTextForTips("有新数据，请刷新");
        }
    }
    ...
    public void setOnRefreshListViewListener(RefreshListViewListener l) {
        mListViewListener = l;
    }
